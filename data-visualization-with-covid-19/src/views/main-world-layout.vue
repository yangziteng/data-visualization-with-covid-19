      ...
      <div class="last-update-choose">
       <router-link to="/" style="color:#fff" >
        ...
        <router-link to="/world" style="color:#fff"  >
          <div class="last-update-choose-2 bottom">全球</div>
        </router-link>
      </div>
      <div class="title">
        <h1>{{ title }}</h1>
        <div class="top-header-tip">
          ...
          <div class="last-update-time">更新时间：（更新时间：2022-12-20）</div>
        </div>
      </div>
    </div>
...
import BasicTrendChart from "../components/BasicTrendWorldChart";
...
import covid19Service from "../api/World.js";
  ...
  methods: {
    ...
    setBasicIncrTrendData(data) {
      ...
      for (let i = data.confirmedCountList.length - 1; i >= 0; i--) {
        dateList.push(data.confirmedCountList[i][1]);
